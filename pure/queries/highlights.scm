...
; PURE-GRAMMAR VARIANT.
; The pure grammar is a strict subset of the full one: identical named node types
; except for four preprocessor nodes it does not produce, because conditional
; compilation is resolved by the preprocessor before this grammar ever sees the
; source. Three of those four are referenced by the full grammar's highlights.scm
; and are omitted here -- `pp_open`, `pp_else_tail`, `pp_end_only`. Leaving them in
; is not a harmless no-op: tree-sitter rejects the pattern as impossible and the
; WHOLE query fails to compile, taking all highlighting with it.
;
; `pp` itself exists in both grammars, so it stays.
(pp) @preproc
